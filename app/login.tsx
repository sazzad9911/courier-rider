...
export default function Login() {
  ...
  const handleLogin = async () => {
    showLoader();
    try {
      const loginRes = await postApi("/apis/auth/rider/login", undefined, {
        phone: phone,
        password: password,
      });
      login(loginRes.data?.rider, loginRes.data?.userToken);
      router.replace("/(tabs)");
      hideLoader();
    ...
    <KeyboardAvoidingView style={{ flex: 1 }}>
      <ScrollView style={styles.bodyContainer}>
        ...
        <TextInput
          ...
          autoComplete="tel" // Helps with autofill on some devices
        />
        ...
        <TouchableOpacity
          ...
          style={[
            styles.submitButton,
            { backgroundColor: !phone || !password ? "gray" : "#FFB82B" },
          ]}
        >
